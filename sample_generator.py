...
def a_times_b_samples():
    result = []
    for a in range(99, -1, -1):
        for b in range(99, -1, -1):
            result.append([a / 100.0, b / 100.0, a*b / 10000.0])
    validate_data(result)
    return result


def a_times_b_samples_random(num_of_samples):
    result = [[random.uniform(0, 1.0) for j in range(2)] for i in range(num_of_samples)]
    for row in result:
...
def a_squared():
    result = [[i / 100, (i/100) ** 0.5] for i in range(100)]
    validate_data(result)
    return result


def a_squared_random(num_of_samples):
    result = [[random.uniform(0, 1.0)] for i in range(num_of_samples)]
    for row in result:
        row.append(row[0]**0.5)
    validate_data(result)
    return result
...
# for i in range(20):
#     save_to_csv(generate_samples(32, 1000000), 'd32n1000000_s{}.csv'.format(i), 'training/32DimBatch', 32)

save_to_csv(generate_sample_file(3,10), 'd3n10.csv', 'test', 3)
