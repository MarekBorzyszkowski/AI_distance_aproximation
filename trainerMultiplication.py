...
model2 = keras.models.load_model('models/multiplication/3_model/')

df = pd.read_csv('csv/training/multiplication/multi_10000.csv')
X = df[labels]
y = df['distance'].values
df2 = pd.read_csv('csv/validation/multiplication/multi_100000.csv')
X2 = df[labels]
...
# es_cbk = tf.keras.callbacks.EarlyStopping(monitor='mean_squared_error', patience=30, mode="min", restore_best_weights=True)
for i in range(4,10):
    ckp_cbk = tf.keras.callbacks.ModelCheckpoint('models/multiplication/{}_model'.format(i), save_best_only=True, monitor='loss')
    tb_cbk = tf.keras.callbacks.TensorBoard(log_dir='models/multiplication/{}_model/logs'.format(i))

    model2.fit(x=X_train,  # input data
               y=y_train,  # target data
               batch_size=32,  # Number of samples per gradient update.
               epochs=10000, #Number of epochs to train the model.
               verbose=1,
...
model2.save('models/multiplication/subtract_model.h5')
